...
pub fn composite_layers(width: u32, height: u32, layers: &[Layer]) -> Vec<u8> {
    ...
    for layer in layers {
        ...
        blend_layer_premul(&mut out, &layer.pixels, layer.opacity.clamp(0.0, 1.0), layer.blend);
    }
...
pub fn premul_to_straight_rgba(premul: &[u8]) -> Vec<u8> {
    ...
    for i in (0..out.len()).step_by(4) {
        ...
        out[i] = ((premul[i] as u32 * 255 + a as u32 / 2) / a as u32).min(255) as u8;
        out[i + 1] = ((premul[i + 1] as u32 * 255 + a as u32 / 2) / a as u32).min(255) as u8;
        out[i + 2] = ((premul[i + 2] as u32 * 255 + a as u32 / 2) / a as u32).min(255) as u8;
    }
    out
}
